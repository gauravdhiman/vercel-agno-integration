/**
 * Script to generate Python code from TypeScript definitions
 * 
 ...
 * 
 * Run with: node generate_python.js
 */
...
const tsFilePath = path.join(__dirname, 'frontend_tools.ts');
...
function generatePythonCode() {
  ...
  // Extract schema generation functions
  const schemaFunctions = [
    { name: 'getAskUserConfirmationSchema', regex: /function getAskUserConfirmationSchema\(\): FrontendToolSchema {([^}]*})}/s },
    { name: 'getDisplayProductCardSchema', regex: /function getDisplayProductCardSchema\(\): FrontendToolSchema {([^}]*})}/s },
    { name: 'getDisplayToolInfoSchema', regex: /function getDisplayToolInfoSchema\(\): FrontendToolSchema {([^}]*})}/s }
  ];
...
const pythonFilePath = path.join(__dirname, 'frontend_tools.py');
